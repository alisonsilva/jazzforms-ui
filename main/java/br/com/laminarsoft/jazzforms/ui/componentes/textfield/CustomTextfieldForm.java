...
public class CustomTextfieldForm extends CustomTextfield implements IJFFormComponent {
...
//		centerBox.setPrefWidth(IJFFormComponent.CENTER_PREF_WIDTH);
		...
		final CustomTextfieldForm atual = this;
			...
			public void handle(MouseEvent event) {
				...
				if (compDesenho == 0) {
					...
					if (event.isControlDown() && event.getClickCount() == 1) {
						...
						dialog.show("Um campo texto não é container para outros componentes", "Ok");
					}
				}
				event.consume();
			}			
