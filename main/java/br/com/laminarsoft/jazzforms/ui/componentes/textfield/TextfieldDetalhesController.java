...
public class TextfieldDetalhesController implements Initializable, IControllerDetalhes {
	...
	@FXML private CheckBox chkTextArea;
	@FXML private CheckBox chkEmailField;
	@FXML private TextField txtPlaceHolder;
	@FXML private NumberSpinner maxLength;
	@FXML private NumberSpinner maxLines;
	
	private CustomTextfield customTextField = null;
	private IDetalhesComponentes customComponent; 
	
	@Override
	public void btnConfirmaAction(ActionEvent actionEvent) {
		customTextField = (CustomTextfield)customComponent.getCustomComponent();
		limpaCampos();
		if (validaEntrada()) {
	        if (customTextField != null) {
		        br.com.laminarsoft.jazzforms.persistencia.model.senchawrapper.Text backComponent = 
		        		(br.com.laminarsoft.jazzforms.persistencia.model.senchawrapper.Text) customTextField.getBackComponent();
		        ...
		        if(chkTextArea.isSelected()) {
		        	backComponent.setTextArea(true);
		        	backComponent.setMaxRows(maxLines.getNumber().intValue());
		        } else {
		        	backComponent.setTextArea(false);
		        	backComponent.setMaxRows(4);
		        }
		        
		        if (chkEmailField.isSelected()) {
		        	backComponent.setEmailField(true);
		        } else {
		        	backComponent.setEmailField(false);
		        }
	...
	public void btnCancelaAction(ActionEvent actionEvent) {
		...
		if(customTextField != null) {
			...
			if (backComponent.getTextArea()) {
				chkTextArea.setSelected(true);
				maxLines.setDisable(false);
				maxLines.setNumber(new BigDecimal(backComponent.getMaxRows() != null ? backComponent.getMaxRows() : 4));
			} else {
				chkTextArea.setSelected(false);
				maxLines.setDisable(true);
				maxLines.setNumber(new BigDecimal(backComponent.getMaxRows() != null ? backComponent.getMaxRows() : 1));
			}
			
			if(backComponent.getEmailField()) {
				chkEmailField.setSelected(true);
			} else {
				chkEmailField.setSelected(false);
			}
	...
	public void initialize(URL url, ResourceBundle resBundle) {
		maxLines.setDisable(true);
		chkTextArea.selectedProperty().addListener(new ChangeListener<Boolean>() {
		    @Override
		    public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
		    	maxLines.setDisable(!newValue);
		    	if(newValue) {
		    		chkEmailField.setSelected(false);
		    	}
		    }
		...
		chkEmailField.selectedProperty().addListener(new ChangeListener<Boolean>() {
	...
	private void inicializacaoCampos() {
		...
		if(customTextField != null) {
			...
			chkEmailField.setSelected(backComponent.getEmailField());
